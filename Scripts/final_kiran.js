...
//var studyIds = ["paac_jhu_2014","laml_tcga_pub"];
...
function getResultMale(){
	...
	if (processDone) {
		  ...
		  for(var key in Obj_Fetch)
						...
						//temp_var.hispanic=Obj_Fetch[key].HISPANIC;
						//temp_var.not_hispanic=Obj_Fetch[key].NOT_HISPANIC;
					...
					.on('load', function(d) {					
						for(var i=0;i<d.length;i++)
						{
							for( key in d[i])
							{
								...
								//var final_obj={cancer_type : "",MALE : "",Female : "",HISPANIC : "",NOT_HISPANIC : ""}
								/*
								
								if(days_to_death>=0)
								{
									var column_value=d[i][key].split("	")[""||days_to_death];
									//cur_row_value=d[i][key].split("	")[key.split("	").indexOf("CANCER_TYPE")];
										...
										if(Obj_Fetch[cur_row_value]== undefined)
										{
											...
											Obj_Fetch[cur_row_value]["days_to_death"]=0;											
											
										}
										// debugger;
										Obj_Fetch[cur_row_value]["days_to_death"]=parseInt(Obj_Fetch[cur_row_value]["days_to_death"])+1;
								...
								if(ethnicity>=0)
								{
									var column_value=d[i][key].split("	")[""||ethnicity];
									cur_row_value=d[i][key].split("	")[key.split("	").indexOf("CANCER_TYPE")];
									if(column_value=="HISPANIC OR LATINO")
									{
										...
										{
											Obj_Fetch[cur_row_value]=[];
											Obj_Fetch[cur_row_value]["HISPANIC OR LATINO"]=0;											
											
										}
										// debugger;
										Obj_Fetch[cur_row_value]["HISPANIC"]=parseInt(Obj_Fetch[cur_row_value]["HISPANIC OR LATINO"])+1;
										
									}
									else if(column_value=="NOT HISPANIC OR LATINO")
									{
										var cur_row_value=d[i][key].split("	")[key.split("	").indexOf("CANCER_TYPE")];
										if(Obj_Fetch[cur_row_value]== undefined)
										{
											Obj_Fetch[cur_row_value]=[];
											Obj_Fetch[cur_row_value]["NOT HISPANIC OR LATINO"]=0;											
											
										}
										Obj_Fetch[cur_row_value]["NOT_HISPANIC"]=parseInt(Obj_Fetch[cur_row_value]["NOT HISPANIC OR LATINO"])+1;
										//debugger;
